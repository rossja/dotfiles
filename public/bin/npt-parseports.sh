#!/bin/bash
function usage {
  echo "usage" $1 [ -f nmap.greppable] [-i IP] [-p port] [-s service] [-P protocol]
}
...
elif [[ ! -z $service ]]; then # search by service
